...
DEFUN_USRATTR(as_tcap_unroutable_sessions, as_tcap_unroutable_sessions_cmd,
	      OSMO_SCCP_LIB_ATTR_RSTRT_ASP,
	      "tcap-unroutable-sessions (reject-udts | load-share-over-as | route-fallback)",
...
DEFUN_USRATTR(as_tcap_unroutable_route_fallback, as_tcap_unroutable_route_fallback_cmd,
	      OSMO_SCCP_LIB_ATTR_RSTRT_ASP,
	      "tcap-route-fallback DPC",
	      "Fallback PC where to route TCAP message of an ongoing session where no valid destination can be found.\n"
	      "The destination Point Code where the MSU are routed\n")
{
	struct osmo_ss7_as *as = vty->index;
...
void ss7_vty_write_one_as(struct vty *vty, struct osmo_ss7_as *as, bool show_dyn_config)
{
	...
	if (as->cfg.loadshare.tcap.unroutable_tcap_fallback_dpc)
		vty_out(vty, "  tcap-route-fallback %s%s",
